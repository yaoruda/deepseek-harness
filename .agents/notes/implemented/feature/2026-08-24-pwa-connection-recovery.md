...
`dsh-client-connection` exposes its coarse connection state as an observable source and owns `reconnect()`. Recovery marks the current generation for immediate retirement, aborts either its live streams or active retry delay, and lets the existing owner loop create the replacement generation. It never starts a second loop or a parallel WebSocket pair.
