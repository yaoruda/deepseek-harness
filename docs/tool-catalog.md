...
| `@deepseek-ai/dsh-tool-hotel-map` | `hotel_map` | `ctx.tools`, `a calling Agent with an owning Session at execution time` | `tool/call`, `travel-map/show after provider work settles`, `tool/result` | - | The tool geocodes known hotel addresses and optionally compares each resolved hotel with one destination; it does not search hotel prices, availability, photos, or bookings. |
