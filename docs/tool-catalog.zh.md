...
| `@deepseek-ai/dsh-tool-hotel-map` | `hotel_map` | `ctx.tools`、执行时具备所属 Session 的调用 Agent | `tool/call`、服务请求结束后的 `travel-map/show`、`tool/result` | - | 该工具解析已知酒店地址，并可选地比较每家已定位酒店到一个目的地的路线；它不搜索酒店价格、可订状态、图片或预订。 |
...
按地址在地图上标记数量受限的已知酒店。可选地比较每家酒店到一个目的地的驾车与公交路线。该工具不搜索价格、可订状态或库存。
...
该工具解析已知酒店地址，并可选地比较每家已定位酒店到一个目的地的路线；它不搜索酒店价格、可订状态、图片或预订。
