...
describe('hotel_map tool', () => {
  ...
  it('appends one complete replay event after both route providers settle', async () => {
    ...
    const result = await ctx.tools.execute({
      signal: new AbortController().signal, callId: ToolCallId('map-call'), name: 'hotel_map', agent,
      arguments: {
        title: 'Stockholm',
    ...
    const event = agent.session.snapshotEvents().findLast(item => item.type === 'travel-map/show')
    ...
    expect(agent.session.snapshotEvents().filter(item => item.type === 'travel-map/show')).toHaveLength(1)
  })
})
